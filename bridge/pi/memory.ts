...
import { extractTurns, ingest, lastUserText, memoryConfig, memoryLogs, memorySearch, pushedKey, recall, setPushedKey } from "./lib.ts"
...
export default function (pi: ExtensionAPI) {
  ...
  // ① session-start recall injection (read side — omitted in input-only mode).
  if (cfg.mode !== "input") {
  ...
  // ② session-end transcript push (write side — omitted in output-only mode).
  // getEntries() is complete here (the aborted turn is settled before
  // session_shutdown fires). Ingest is deduped against the on-disk pushed
  // fingerprint so a resumed session (same id, unchanged transcript) skips.
  if (cfg.mode !== "output") {
