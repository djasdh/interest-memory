/**
 * interest-memory bridge — shared logic for Claude Code hooks.
 *
...
export function memoryConfig(env = process.env) {
  ...
  return {
    baseUrl: base,
    agent: env.INTEREST_AGENT || "claudecode",
...
/**
 * Reduce a Claude Code transcript jsonl (OpenAI wire format) to
 * `[{role, content}]` wire turns. Only user/assistant text kept.
 */
export function parseClaudeTranscript(path) {
  const out = []
